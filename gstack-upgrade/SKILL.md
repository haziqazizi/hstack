...
[ -z "$_AUTO" ] && _AUTO=$("$_GS/bin/gstack-config" get auto_upgrade 2>/dev/null || true)
...
```bash
"$_GS/bin/gstack-config" set auto_upgrade true
```
...
**If "Never ask again":**
```bash
"$_GS/bin/gstack-config" set update_check false
```
Tell user: "Update checks disabled. Run `gstack-config set update_check true` in your gstack skills directory to re-enable."
...
```bash
"$_GS/bin/gstack-update-check" --force 2>/dev/null || \
"$_GS_LOCAL/bin/gstack-update-check" --force 2>/dev/null || true
```
