...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `_GS=$([ -d "$HOME/.agents/skills/gstack" ] && echo "$HOME/.agents/skills/gstack" || echo "$HOME/.claude/skills/gstack")
source <("$_GS/bin/gstack-diff-scope" <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
1. Read `$AGENT_DIR/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
_GS=$([ -d "$HOME/.agents/skills/gstack" ] && echo "$HOME/.agents/skills/gstack" || echo "$HOME/.claude/skills/gstack")
source <("$_GS/bin/gstack-diff-scope" <base> 2>/dev/null)
```
...
2. **Read the design checklist.** Check `$AGENT_DIR/skills/review/design-checklist.md` (where `$AGENT_DIR` is `.agents` or `.claude`, whichever exists). Also check `$GSTACK_DIR/../review/design-checklist.md`. If the file cannot be read from either location, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
_GS=$([ -d "$HOME/.agents/skills/gstack" ] && echo "$HOME/.agents/skills/gstack" || echo "$HOME/.claude/skills/gstack")
"$_GS/bin/gstack-review-log" '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
Read `$AGENT_DIR/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
```bash
"$_GS/bin/gstack-review-log" '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE"}'
```
...
Read `$AGENT_DIR/skills/review/TODOS-format.md` for the canonical format reference.
