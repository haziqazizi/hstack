# hstack (fork of gstack)

This repository is **hstack**, my personal fork of [Garry Tan's gstack](https://github.com/garrytan/gstack). The product and slash-command surface still use the **gstack** name for compatibility, so install paths remain `~/.claude/skills/gstack` and project copies still live in `.claude/skills/gstack`.
...
**Requirements:** [Claude Code](https://docs.anthropic.com/en/docs/claude-code), [Git](https://git-scm.com/), [Bun](https://bun.sh/) v1.0+, and [`agent-browser`](https://www.npmjs.com/package/agent-browser) installed via `npm install -g agent-browser && agent-browser install`.

### Step 1: Install on your machine

Open Claude Code and paste this. Claude does the rest.

> Install gstack from this fork: run **`git clone https://github.com/haziqazizi/hstack.git ~/.agents/skills/gstack && cd ~/.agents/skills/gstack && ./setup`** then add a "gstack" section to AGENTS.md (or CLAUDE.md) that says to use the /browse skill from gstack for all web browsing, never use mcp\_\_claude-in-chrome\_\_\* tools, and lists the available skills: /office-hours, /plan-ceo-review, /plan-eng-review, /plan-design-review, /design-consultation, /docs-research, /review, /ship, /browse, /qa, /qa-report, /design-review, /setup-browser-cookies, /retro, /investigate, /compound, /document-release, /codex, /careful, /freeze, /guard, /unfreeze, /gstack-upgrade. Then ask the user if they also want to add gstack to the current project so teammates get it.

> **Already have it at `~/.claude/skills/gstack`?** That still works — hstack checks both locations automatically.

### Step 2: Add to your repo so teammates get it (optional)

> Add gstack to this project: run **`cp -Rf ~/.agents/skills/gstack .agents/skills/gstack && rm -rf .agents/skills/gstack/.git && cd .agents/skills/gstack && ./setup`** then add a "gstack" section to this project's AGENTS.md (or CLAUDE.md) that says to use the /browse skill from gstack for all web browsing, never use mcp\_\_claude-in-chrome\_\_\* tools, lists the available skills: /office-hours, /plan-ceo-review, /plan-eng-review, /plan-design-review, /design-consultation, /docs-research, /review, /ship, /browse, /qa, /qa-report, /design-review, /setup-browser-cookies, /retro, /investigate, /compound, /document-release, /codex, /careful, /freeze, /guard, /unfreeze, /gstack-upgrade, and tells the agent that if gstack skills aren't working, run `cd .agents/skills/gstack && ./setup` to build the binary and register skills.

Real files get committed to your repo (not a submodule), so `git clone` just works. Everything lives inside `.agents/` (or `.claude/`). Nothing touches your PATH or runs in the background.
...
Seventeen specialists and six power tools. All slash commands. All Markdown. All free. **Fork:** [github.com/haziqazizi/hstack](https://github.com/haziqazizi/hstack) · **Upstream:** [github.com/garrytan/gstack](https://github.com/garrytan/gstack) — MIT License
