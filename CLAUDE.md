...
bun test             # run free tests (skill validation + generator quality + helper/unit tests)
...
`bun test` runs skill validation, gen-skill-docs quality checks, and helper/unit
tests. `bun run test:evals` runs LLM-judge quality evals and E2E
...
```
gstack/
├── browse/          # /browse skill docs and examples (powered by agent-browser)
│   ├── bin/         # Small helper scripts still used by other skills (e.g. remote-slug)
│   └── SKILL.md.tmpl
├── bin/             # Helper CLIs (gstack-config, gstack-cookie-import, etc.)
├── scripts/         # Build + DX tooling
│   ├── gen-skill-docs.ts  # Template → SKILL.md generator
│   ├── skill-check.ts     # Health dashboard
│   └── dev-skill.ts       # Watch mode
├── test/            # Skill validation + eval tests
│   ├── helpers/     # skill-parser.ts, session-runner.ts, test-server.ts, llm-judge.ts, eval-store.ts
│   ├── fixtures/    # Ground truth JSON, browser fixtures, eval baselines
...
├── docs-research/    # /docs-research skill (stack-aware docs + web research)
├── compound/         # /compound skill (capture incidents + durable guardrails)
...
└── package.json     # Build/test scripts for skill generation and evals
```
...
To change the browser command docs or snapshot guidance, edit `scripts/gen-skill-docs.ts`
and the relevant `.tmpl` files, then run `bun run gen:skill-docs`.
...
`/browse` skill or run agent-browser directly via the session alias shown in the generated
SKILL docs (`$AB <command>`). NEVER use
...
3. Regenerate docs if needed: `cd ~/.claude/skills/gstack && bun run build`
