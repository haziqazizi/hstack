...
1. Read the project agent config dir (`$AGENT_DIR/stack.yaml`) if present.
2. Read the relevant files in `$AGENT_DIR/architecture/rules/` for the detected or proposed stacks.
3. Read the most relevant memos in `$AGENT_DIR/research/` and patterns in `$AGENT_DIR/compound/patterns/`.
...
- the project agent config dir (`$AGENT_DIR/stack.yaml`)
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `$AGENT_DIR/skills/review/TODOS-format.md`.
...
```bash
"$_GS/bin/gstack-review-log" '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
